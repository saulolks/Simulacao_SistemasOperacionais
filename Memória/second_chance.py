...
class Manager:
    ...
    def run(self):
        for process in self.processes:
            thread = threading.Thread(target=self.processing, args=(process,))
            thread.run()
    ...
    def processing(self, process):
        while True:
            ...
            print(f"{result} recovered from memory!")

            time.sleep(0.8)
    
    def show(self):
        border1 = "=" * int(len(self.main_memory) * 5.2)
